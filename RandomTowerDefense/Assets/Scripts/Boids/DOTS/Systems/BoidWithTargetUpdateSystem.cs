...
public class BoidWithTargetUpdateSystem : JobComponentSystem
{
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        return Entities.WithAll<BoidTag>().ForEach((Entity entity, ref BoidDirection forward, ref OriPos oripos,
            ref Target targetType,
            ref Translation transformType, ref BoidData dataType, ref Velocity vecType,
            ref BoidSettingDots settingType) => {
                Vector3 acceleration = Vector3.zero;

                Vector3 offsetToTarget = (targetType.targetPos - transformType.Value);
                acceleration = SteerTowards(offsetToTarget,
                    new Vector3(vecType.Value.x, vecType.Value.y, vecType.Value.z), settingType) * settingType.targetWeight;
    ...
    public static Vector3 SteerTowards(Vector3 vector, Vector3 velocity, BoidSettingDots settings)
    {
