...
public class MeteorBehaviour : MonoBehaviour, IConvertGameObjectToEntity
{
	private readonly int TotalNum = 1000;
	public static MeteorBehaviour Instance { private set; get; }
	...
	{
		var archeType = manager.CreateArchetype(typeof(Hybrid), typeof(SkillTag), typeof(MeteorTag),
		...
		float wait, float cycle, float exp, int level, float3 targetpos, int targetid)
	{
		...
		MeteorBehaviour.skWait[id] = wait;
		MeteorBehaviour.skActive[id] = active;
		MeteorBehaviour.skCycle[id] = cycle;
		MeteorBehaviour.skAction[id] = action;
		MeteorBehaviour.skArea[id] = area;
		MeteorBehaviour.skDamage[id] = damage;
		MeteorBehaviour.skExp[id] = exp;
		MeteorBehaviour.skLevel[id] = level;
		MeteorBehaviour.skId[id] = id;
		MeteorBehaviour.skTargetPos[id] = targetpos;
		MeteorBehaviour.skTargetID[id] = targetid;
	}
