...
public class FluidCtrl
{
	...
	public static bool colorful_enable = true;
	public static bool paused_check = false;
	public static bool bloom_enabled = true;
	public static bool autohit_enable = false;
	...
	FluidProgram clearProgram, displayProgram, displayBloomProgram, bloomPrefilterProgram, bloomBlurProgram, bloomFinalProgram, splatProgram, advectionProgram,
	...
	ShaderBuildinHelper shaderBuildinHelper;
	public FluidCtrl(RenderTexture target, Texture2D _ditheringTexture, ShaderBuildinHelper _shaderBuildinHelper)
	{
		ditheringTexture = _ditheringTexture;
		shaderBuildinHelper = _shaderBuildinHelper;
		if (ditheringTexture == null)
		{
			ditheringTexture = Resources.Load("LDR_RGB1_0") as Texture2D;
		}
		renderTarget = target;
		if (target == null)
			return;
		InitFramebuffers(renderTarget);
		InitProgram(shaderBuildinHelper);
	}
	public static void SwithBackgroundColor()
	{
		BACK_COLOR = GenerateColor();
	}
	public bool CouldProcess()
	{
		return renderTarget != null;
	}
	// Update is called once per frame
	public RenderTexture Process(float dt)
	{
		if (drawcount <= 0)
			return renderTarget;
		drawcount--;
	...
	void InitFramebuffers(RenderTexture target)
	{

		screen_w = target.width;
		screen_h = target.height;
		Vector2 simv = GetResolution(target, sim_resolution);
		Vector2 dyev = GetResolution(target, dye_resolution);
		simWidth = (int)simv.x;
		simHeight = (int)simv.y;
		dyeWidth = (int)dyev.x;
		dyeHeight = (int)dyev.y;
		Debug.Log("simWidth:" + simWidth + "  simHeight:" + simHeight + "  dyeWidth:" + dyeWidth + "  dyeHeight:" + dyeHeight);
	...
	Vector2 GetResolution(RenderTexture target, float resolution)
	{
