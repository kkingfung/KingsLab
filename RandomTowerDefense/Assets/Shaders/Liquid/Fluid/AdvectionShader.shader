Shader "Custom/Fluid/AdvectionShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 coord = i.uv - _dt * tex2D(_Velocity, i.uv).xy * float2(_texelSize_x,_texelSize_y);
                return float4((_dissipation * tex2D(_Source, coord)).xyz, 1.0);
            }
            ENDCG
        }
    }
